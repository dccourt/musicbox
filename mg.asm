...
; Minor tweaks by Fanjita, 2012. 

;.include "tn45.inc"	;This is included in "Atmel AVR Studio"
.device attiny85

; gavrasm doesn't define these automatically - grabbed from tn45def.inc.
...
;	outi	OSCCAL, 172		;Adjust OSCCAL if needed.
	...
	outi	PORTB, 0b000101		;Initalize Port B. logic high on unused pins, low on the outputs we're using.
	outi	DDRB,  0b011010		;/   - pins 1, 3, 4 as output (speaker across 1 and 4, LED on 3).
...
song_finished:
	; to loop endlessly:   rump start_play.
	; instead, we're going to sleep:
	; Select sleep mode: (deep sleep: SM1=1, SM0=0)
	; and enable sleeping (SE=1).
	ldi 	r16, (1<<SM1) | (1<<SE)
;	in	r16, MCUCR
;	ori	r16, (1<<SE)
	out	MCUCR, r16  ; set sleep_enable

	; just before we sleep, turn off the LED on PB3 (low=on).
	outi	PORTB, 0b001101
...
; seems this addiw opcodes isn't compiled correctly, so I've hacked my own macro.  Problem is negative sign extension in the high byte in gavrasm - so use a macro here that expects an operand of <= 255.
	addiw_byte	Y, Notes
	ldiw	B, wt_attack*2
...
dcskip4:
	...
	movw	T2L, EL			;
dcskip5:
;	ldiw	E, -255			; this doesn't assemble properly in gavrasm, expand macro manually and fix:
	ldi	EL,low(-255)
	ldi	EH,255
